...
Purpose of this repository is to show how automatic build and deployment can be done in GCP environment, using GitHub Actions and Terraform Cloud.
...
- main branch: deployment without terraform, using kubectl
- gke-terraform-deploy branch: deployment with only terraform
...
To be able to replicate terraform pipeline:
...
2. Create your own namespace within your [Terraform Cloud organisation](https://app.terraform.io/):
    ...
    - Set Namespace Environment Variables:
        - GOOGLE_CREDENTIALS (check: sensitive, paste here your GCP service account json credentials, you have to remove blank files from that file or it won't work)
    - In your account profile, go to User Settings -> Tokens. Create token that will be used by GitHub Actions. Save the value for later use.
...
4. On repository level, go to Settings:
    - On Secrets tab, create following secrets:
        - GCLOUD_SERVICE_KEY -> base64 encoded json file content (your service account credentials)
        - TF_API_TOKEN -> token that you copied from Terraform Cloud
        - REPO_ACCESS_TOKEN -> on your GitHub account level/organisation level, create access token that can read repository information and trigger workflows - paste it here. This is needed for automatic trigger of validation pipeline. (if you don't want to do that, you can skip it)
    - On Environments tab:
...
6. On validation repo:
    - Go to Repository Settings tab -> Secrets, and add:
        - GCP_CREDENTIALS -> base64 encoded json file content (your service account credentials)
